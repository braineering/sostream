...
public class ActiveEventPostCommentMapper2 implements CoFlatMapFunction<EventQueryOne, Long, EventQueryOne> {
	
	private static final long serialVersionUID = 1L;
	
	private static final Logger LOGGER = LoggerFactory.getLogger(ActiveEventPostCommentMapper2.class.getSimpleName());	
	
	private CommentMap comment2Post;
	
	private List<Long> bufferRemoval;
	
	public ActiveEventPostCommentMapper2(){
	...
	public void flatMap1(EventQueryOne event, Collector<EventQueryOne> out) throws Exception {
		...
		if (this.bufferRemoval.size() > 1000) {
	...
	@Override
	public void flatMap2(Long postId, Collector<EventQueryOne> out) throws Exception {
		this.bufferRemoval.add(postId);
