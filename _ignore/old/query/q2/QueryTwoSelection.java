...
import com.threecore.project.operator.rank.comment.CommentRankMergerSelection;
import com.threecore.project.operator.rank.comment.CommentRankUpdateFilter;
import com.threecore.project.operator.rank.comment.CommentRankerSelection;
...
public class QueryTwoSelection {	

	private static final String JOB_NAME = "q2-selection";	
	...
	public static void main(String[] args) throws Exception {		
		...
		DataStream<CommentRank> bests = scores.keyBy(new CommentScoreKeyer()).flatMap(new CommentRankerSelection(config.getK())).setParallelism(config.getParallelism());
		...
		if (config.getParallelism() == 1) {
			...
			tops = bests.flatMap(new CommentRankMergerSelection(config.getK())).setParallelism(1);
		}
