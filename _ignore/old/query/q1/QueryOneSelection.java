...
import com.threecore.project.operator.rank.post.PostRankMergerSelection;
import com.threecore.project.operator.rank.post.PostRankUpdateFilter;
import com.threecore.project.operator.rank.post.PostRankerSelection;
...
public class QueryOneSelection {	

	private static final String JOB_NAME = "q1-selection";
	...
	public static void main(String[] args) throws Exception {		
		...
		DataStream<PostRank> bests = scoresMain.keyBy(new PostScoreKeyer()).flatMap(new PostRankerSelection()).setParallelism(config.getParallelism());
		...
		if (config.getParallelism() == 1) {
			...
			tops = bests.flatMap(new PostRankMergerSelection()).setParallelism(1);
		}
