...
public class QueryTwo {	
	...
	public static void main(String[] args) throws Exception {		
		...
		if (debug) scores.addSink(new AsStringSink<CommentScore>(config.getSinkPath(JOB_NAME, "scores")));
		...
		if (debug) bests.addSink(new AsStringSink<CommentRank>(config.getSinkPath(JOB_NAME, "bests")));	
		...
		if (debug) tops.addSink(new AsStringSink<CommentRank>(config.getSinkPath(JOB_NAME, "tops")));	

		DataStream<CommentRank> newtops = tops.filter(new CommentRankUpdateFilter(config.getK())).setParallelism(1);

		newtops.addSink(new AsStringSink<CommentRank>(config.getSinkPath(JOB_NAME, "out")));
